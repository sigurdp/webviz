...
class TempUserStore:
    ...
    async def put_bytes_async(self, key: str, payload: bytes, blob_prefix: str | None, blob_extension: str) -> bool:
        ...
        async with self._goc_container_client_async() as container_client:
            await _upload_or_refresh_blob_metadata_async(container_client, blob_name, payload)
    ...
    async def get_bytes_async(self, key: str) -> bytes | None:
        ...
        async with self._goc_container_client_async() as container_client:
            payload_bytes = await _download_blob_async(container_client, blob_name)
    ...
    async def put_pydantic_model_async(
        ...
        ret_val = await self.put_bytes_async(key, payload, blob_prefix=blob_prefix, blob_extension=blob_extension)
    ...
    @asynccontextmanager
    async def _goc_container_client_async(self) -> AsyncIterator[ContainerClient]:
...
async def _upload_or_refresh_blob_metadata_async(container_client: ContainerClient, blob_name: str, payload: bytes) -> str:
...
async def _download_blob_async(container_client: ContainerClient, blob_name: str) -> bytes | None:
