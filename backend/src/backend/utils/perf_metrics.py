...
class PerfMetrics:
    ...
    def register_lap(self, metric_name: str):
        """Registers metric with a duration since the last lap"""
        self.set_metric(metric_name, self._perf_timer.lap_ms())

    def register_elapsed(self, metric_name: str):
        """Registers metric with a duration since the start"""
